...
from ml.model import inference, load_lb, load_model,\
                     load_encoder, compute_model_metrics
...
def slice_data(df, feature):
    ...
    for cls in df[feature].unique():
        ...
        X, y, _, _ = process_data(df_temp, categorical_features=cat_features,
                                  label="salary", training=False,
                                  encoder=encoder, lb=lb)
        preds = inference(model, X)
        precision, recall, fbeta = compute_model_metrics(lb.transform(y),
                                                         preds)
        print(f"Precision for {feature} =={cls}:  {precision}")
        print(f"Recall for {feature} =={cls}:  {recall}")
        print(f"Fbeta for {feature} =={cls}:  {fbeta}")
        line = {"class": cls, "precision": precision,
